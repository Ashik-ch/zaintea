...
import { Flame, Zap, Coffee, Clock, Plus, Utensils, Star, CupSoda, Instagram, Globe, Phone } from "lucide-react";
import { motion } from "framer-motion";
import { useCart } from "../context/CartContext";
import Cart from "./Cart";
import bgImg from '../assets/greenBg.jpg';
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden overflow-y-auto font-sans text-slate-900 flex flex-col px-4 pt-2 pb-8 relative">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-transform duration-700 blur-sm scale-105"
          style={{ backgroundImage: `url(${bgImg})`, }} />
        {/* <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-transform duration-700"
          style={{
            backgroundImage: "url('https://i.pinimg.com/1200x/d8/d1/32/d8d132934febc374173761c1fd12b620.jpg')",
          }}
        /> */}
        <div className="absolute inset-0 bg-white/10 backdrop-brightness-110" />

        {/* Keep the animated mesh gradients from before for extra depth */}
        <motion.div
              ...
              {specials.map((s) => (
                <div key={s.id} className="group bg-white/40 backdrop-blur-xl border border-white/70 p-4 rounded-[2rem] flex justify-between items-center relative overflow-hidden shadow-sm hover:shadow-xl hover:bg-white/60 transition-all duration-300">
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <button
                      onClick={() => addToCart({ id: s.id, name: s.name, desc: s.desc, price: `AED ${s.price}` }, 1, null)}
                      className="bg-white/80 hover:bg-red-600 hover:text-white p-2.5 rounded-2xl shadow-sm transition-all active:scale-95"
                    >
                      <Plus size={18} />
                    </button>
                  </div>
                  ...
                  <div key={i} className="group flex justify-between items-center px-6 py-3 hover:bg-white/40 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="bg-white/80 group-hover:bg-red-600 group-hover:text-white p-1.5 rounded-xl transition-all shadow-sm"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
